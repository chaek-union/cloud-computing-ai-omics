...
EBS의 볼륨 타입도 무작정 크게만 잡으면 되는 문제는 아니다. 일반적인 부팅 디스크나 보통의 분석 작업은 범용 SSD 기반 볼륨으로 충분한 경우가 많지만, IOPS와 처리량 병목이 확인되는 workflow에서는 더 높은 성능의 볼륨 구성이 필요할 수 있다 (AWS 2026b). 그러나 교육 단계에서 더 중요한 것은 볼륨 타입의 세부 옵션보다 데이터 배치 원칙을 정확히 익히는 일이다. 즉 "원본은 S3, 실행 중 작업은 EBS 또는 scratch, 종료 전 결과는 S3"라는 기본 구조가 먼저다.
