...
이 장의 결론은 단순하다. EC2를 잘 쓰는 사람은 가장 큰 인스턴스를 고르는 사람이 아니라, 병목에 맞는 인스턴스를 고르고, 저장 계층을 분리하고, 권한을 Role로 관리하고, 끝난 자원을 정리할 줄 아는 사람이다. 즉 실전 AWS 운용의 출발점은 화려한 서비스 조합이 아니라, `계산`, `저장`, `권한`, `생명주기`를 분리해 생각하는 습관이다. 이 기본기가 있어야 다음 장에서 다룰 자동화와 대규모 오케스트레이션도 비용과 재현성을 모두 잡는 방향으로 이어질 수 있다.
