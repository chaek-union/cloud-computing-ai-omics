# Chapter 12. SageMaker로 배우는 오믹스 머신러닝
...
## 12.1 SageMaker를 오믹스 분석에서 어디에 놓아야 하는가
...
## 12.2 Notebook, Studio, Unified Studio, 학습 작업은 어떻게 다른가
...
## 12.3 Amazon Genomics CLI와 Nextflow 결과를 SageMaker로 연결하기
...
## 12.4 1000 Genomes 예제로 보는 tertiary analysis
...
## 12.5 실용 모델에서 genomic foundation model까지
...
이 장의 결론은 단순하다. SageMaker는 오믹스 분석의 전 과정을 혼자 담당하는 서비스가 아니다. 대신 S3와 HealthOmics에 정리된 데이터, Batch와 Nextflow가 만든 결과, Athena가 좁혀 준 cohort를 바탕으로, 탐색에서 학습과 배포까지 이어지는 관리형 ML 계층을 제공한다. 따라서 오믹스에서 SageMaker를 잘 쓴다는 것은 "클라우드에 Jupyter를 띄운다"가 아니라, 데이터 파이프라인과 모델 파이프라인의 경계를 명확히 세우고 그 사이를 안정적으로 연결하는 일에 가깝다.
